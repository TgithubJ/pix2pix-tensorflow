...
parser.add_argument("--scale_size", type=int, default=544, help="scale images to this size before cropping to 256x256")
parser.add_argument("--flip", dest="flip", action="store_true", help="flip images horizontally")
parser.add_argument("--no_flip", dest="flip", action="store_false", help="don't flip images horizontally")
parser.set_defaults(flip=True)
parser.add_argument("--lr", type=float, default=0.0002, help="initial learning rate for adam")
parser.add_argument("--lr_decay_rate", type=float, default=0.2, help="initial learning rate for adam")
parser.add_argument("--lr_decay_step", type=int, default=200000, help="initial learning rate for adam")
...
Model = collections.namedtuple("Model", "outputs, gen_loss, gen_grads_and_vars, lr_rate,train")
...
def create_generator(generator_inputs, generator_outputs_channels, dropout_rate):
    ...
    layer_specs = [
        (a.ngf * 8, dropout_rate),   # decoder_8: [batch, 1, 1, ngf * 8] => [batch, 2, 2, ngf * 8 * 2]
        (a.ngf * 8, dropout_rate),   # decoder_7: [batch, 2, 2, ngf * 8 * 2] => [batch, 4, 4, ngf * 8 * 2]
        (a.ngf * 8, dropout_rate),   # decoder_6: [batch, 4, 4, ngf * 8 * 2] => [batch, 8, 8, ngf * 8 * 2]
...
def create_model(inputs, targets, dropout):
    with tf.variable_scope("generator") as scope:
        outputs = create_generator(inputs, generator_outputs_channels=NUM_OF_CLASSESS, dropout_rate=dropout)
    ...
    with tf.name_scope("generator_loss"):
        ...
        # gen_loss = tf.reduce_mean(tf.abs(targets - outputs))
        # gen_loss = gen_loss_GAN * a.gan_weight + gen_loss_L1 * a.l1_weight

        cross_entropies = tf.nn.softmax_cross_entropy_with_logits(
            logits=outputs,
            labels=targets)
        gen_loss = tf.reduce_mean(cross_entropies)
    ...
    with tf.name_scope("generator_train"):
        ...
        learning_rate = tf.train.exponential_decay(
            ...
            decay_steps=a.lr_decay_step, 
            decay_rate=a.lr_decay_rate, 
            staircase=True,
            name="learning_rate")  

        gen_tvars = tf.trainable_variables()
...
def main():
    ...
    if a.mode == "export":
        ...
        output = tf.convert_to_tensor([tf.encode_base64(output_data)], name="output_base64")
        ...
        with tf.Session() as sess:
            ...
            model_exporter.init(
                ...
                named_graph_signatures={
                    'inputs': exporter.generic_signature({'x': input}),
                    'outputs': exporter.generic_signature({'predictions': output})})

            model_exporter.export(
                export_dir_base=exports_dir, 
   ...
   # Image summary.
    ...
    with tf.name_scope("convert_inputs"):
        converted_inputs = depreprocess(inputs, num_images=3, img_mean=IMG_RGB_MEAN, img_std=IMG_RGB_STD) 
    ...
    with tf.name_scope("convert_outputs"):
        ...
        converted_outputs = (outputs * (np.round(256 / (NUM_OF_CLASSESS-1))-1))
    ...
    with sv.managed_session() as sess:
        ...
        if a.mode == "test":
            ...
            for step in range(max_steps):
                ...
                results = sess.run(fetches, feed_dict={final_dropout:0.5} ,options=options, run_metadata=run_metadata)
