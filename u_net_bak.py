...
Model = collections.namedtuple("Model", "outputs, gen_loss, gen_grads_and_vars, train")
...
def load_examples():
    ...
    with tf.name_scope("load_images"):
        ...
        if a.lab_colorization:
            ...
            print("raw_input:", raw_input.get_shape())
            a_images = tf.image.convert_image_dtype(raw_input[:,:width//2,:], dtype=tf.float32)
...
def create_generator(generator_inputs, generator_outputs_channels):
    ...
    layer_specs = [
        (a.ngf * 8, 0.1),   # decoder_8: [batch, 1, 1, ngf * 8] => [batch, 2, 2, ngf * 8 * 2]
        (a.ngf * 8, 0.1),   # decoder_7: [batch, 2, 2, ngf * 8 * 2] => [batch, 4, 4, ngf * 8 * 2]
        (a.ngf * 8, 0.1),   # decoder_6: [batch, 4, 4, ngf * 8 * 2] => [batch, 8, 8, ngf * 8 * 2]
...
def create_model(inputs, targets):
    with tf.variable_scope("generator") as scope:
        outputs = create_generator(inputs, generator_outputs_channels=NUM_OF_CLASSESS)
...
def main():
    ...
    model = create_model(final_input, examples.targets)
